...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="unit">
        <fo:block id="{fn:generate-id()}"
            ...
            <!-- If there is only one presentation in the unit, don't bother with contents-->
            <xsl:if test="fn:count(exercise|presentation) &gt; 1">
                <xsl:for-each select="exercise|presentation">
                    <fo:block start-indent="2em"
                              ...
                              space-before.conditionality="retain"
                              font-weight="normal">
                        <!-- Once exercises were formatted in-line. No longer, but their title should still appear in the Unit TOC.
                                  This is accomplished by the barbarity below.-->

                        <xsl:value-of
                                select='fn:concat(fn:position(), ". ",fn:document(@fileref,.)/presentation/title,
                                    fn:document(handout/@fileref,.)/article/articleinfo/title)'/>
                    </fo:block>
                </xsl:for-each>
            </xsl:if>
        </fo:block>
        ...
        <fo:block>
            <!-- New-style exercises will never be processed, as it's the handout element that has a fileref,
            so the document function beow will only include presentation documents, not exercises.-->
            <xsl:for-each select="exercise|presentation">
    ...
    <!-- Again, new-style exercises will never be processed, as it's the handout element that has a fileref..-->
